...
keymage.$('list', 'defmod-up', function (e) { if (current)
    moveTopOrUp(e, current.pager, current); });
keymage.$('list', 'defmod-down', function (e) { if (current)
    moveBottomOrDown(e, current.pager, current); });
keymage.$('list', ['left', 'shift-up'], function (e) { if (current)
    moveLeft(e, current.pager, current); });
keymage.$('list', ['right', 'shift-down'], function (e) { if (current)
    moveRight(e, current.pager, current); });
keymage.$('list', ['defmod-left', 'defmod-shift-up'], function (e) { if (current)
    pageFirst(e, current.pager, current); });
keymage.$('list', ['defmod-right', 'defmod-shift-down'], function (e) { if (current)
    pageLast(e, current.pager, current); });
keymage.$('list', 'shift-space', function (e) { if (current)
    pageSort(e, current.pager, current); });
keymage.$('list', 'defmod-space', function (e) { if (current)
    ...
    this._dtap = function (e, dbltap, flagsIntersect) {
        ...
        if (flagsIntersect) {
            if (!(pojo.vstate & 16 /* UPDATE */)) {
                ...
                self.pager.store.select(pojo, 3 /* CLICKED_UPDATE */, vm.$index);
            }
        }
        else if (dbltap && trigger) {
            self.pager.store.select(pojo, 1 /* CLICKED */, vm.$index);
        }
        else {
            pojo.vstate ^= 16 /* UPDATE */;
            self.pager.store.select(pojo, 3 /* CLICKED_UPDATE */, vm.$index);
