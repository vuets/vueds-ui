import { screen, isInput, resolveElement, resolveNextPageIdx, vmLookup, fireEvent, table_compact_columns, selectIdx, pageAndSelectIdx, tableUp, tableDown, tableLeft, tableRight, tableJumpUp, tableJumpDown, tableJumpLeft, tableJumpRight } from '../util';
...
function pageNextOrLoad(pager, e) {
    ...
    if (pager.page < pager.page_count) {
        ...
        var array = pager.array;
        pageAndSelectIdx(page, resolveNextPageIdx(page, pager.index_selected, array, pager), array, store, false);
        return;
    }
    // page push
    if ((current.$flags & 1) || (pager.state & 264 /* MASK_RPC_DISABLE */) || !pager.index_hidden)
        return;
    e.preventDefault();
    if (pager.state & 16 /* DESC */) {
        store.requestOlder();
...
export function bind() {
    ...
    this._swipe = function (e) {
        var target = e.target, vm;
        if (isInput(target))
            return;
        current = self;
        keymage.setScope('list');
        vm = vmLookup(target);
        if (!vm || !vm.hasOwnProperty('$index') || (vm !== self.vm && vm.$parent !== self.vm))
            return;
        switch (e.direction) {
            case 2:
    ...
    this._dtap = function (e, dbltap, flagsIntersect) {
        var target = e.target, parent = target.parentElement, vm = vmLookup(target, parent), trigger = target.getAttribute('dtap') || (target = parent).getAttribute('dtap');
        if (trigger)
