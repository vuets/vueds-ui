...
export function pagePrevOrLoad(e, pager, opts) {
    ...
    // page unshift
    if ((opts.flags & 128 /* NO_RPC */) || (pager.state & 264 /* MASK_RPC_DISABLE */))
        return;
...
export function pageNextOrLoad(e, pager, opts) {
    ...
    if (page < pager.page_count) {
        ...
        page = ++pager.page;
        if (opts.flags & 32 /* PAGE_AND_SELECT */) {
            pageAndSelectIdx(page, resolveNextPageIndex(page, pager.index_selected, pager), pager.array, store, opts.flags); // TODO pass flag?
        }
        else {
            store.repaint();
        }
    ...
    // page push
    if ((opts.flags & 128 /* NO_RPC */) || (state & 264 /* MASK_RPC_DISABLE */) || !pager.index_hidden)
        return;
    e.preventDefault();
    if (state & 16 /* DESC */)
        store.requestOlder();
