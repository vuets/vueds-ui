...
function selectItemNT() {
    ...
    selected_item['_'].lstate = 1 /* INCLUDED */ | 2 /* SELECTED */;
}
function selectDayNT() {
    ...
    this.pstore.select(this.pager.array[idx], 4 /* FORCE */);
}
...
export var Calendar = (function () {
    ...
    Calendar.created = function (self) {
        instance = self;
        var today = new Date(), year = today.getUTCFullYear(), month = today.getUTCMonth(), day = today.getUTCDate(), startDate = { year: year, month: month, day: day }, opts = { weekStart: 0 }, cache = {}, current_entry = getEntry(year, month, opts, startDate, cache);
    ...
    Calendar.prototype.page = function (next) {
        repaint(this, next);
    };
    return Calendar;
}());
