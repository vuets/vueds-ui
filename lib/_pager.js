...
export function attachOptsTo(el, args, pager, vm) {
    var i = 0, len = !args ? 0 : args.length, flags = i === len ? 0 : parseInt(args[i++], 10), table_flags = i === len ? 0 : parseInt(args[i++], 10), col_size = table_flags === 0 ? 0 : table_compact_columns(), hammer = new Hammer(el);
    var opts = {
        flags: flags,
        col_size: col_size,
...
function press(e) {
    var opts = this;
    if (isInput(e.target) || (opts.flags & 16 /* SUGGEST */))
        return;
    current = opts;
    keymage.setScope('pager');
...
function tap(e) {
    ...
    var pager = opts.pager, pojo, store, 
    //key,
    suggest;
