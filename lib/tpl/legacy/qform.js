...
export function field_enum(fd, pojo, display, custom_class) {
...
export function field_bool(fd, pojo, display, custom_class) {
...
export function field_suggest(fd, pojo, display, custom_class) {
...
export function field_num(fd, pojo, display, custom_class) {
    return "\n<div class=\"" + (custom_class || 'ui input') + "\">\n  <input type=\"text\"" + (fd.o === 2 && dpicker(false, pojo, fd.$ || fd._) || '') + "\n      placeholder=\"" + display + "\" v-sclass:disabled=\"" + pojo + ".disable_\"\n      v-disable=\"" + pojo + ".disable_\" v-sval:" + fd.t + append(fd.o, ',') + "=\"" + pojo + "." + fd.$ + "\"\n      @change=\"qform.$d.$change($event, " + pojo + ", " + fd._ + ", false, null, " + pojo + "$$)\" />\n</div>";
}
export function field_num_end(fd, pojo, display, custom_class) {
...
export function field_default(fd, pojo, display, changeSuffix, custom_class) {
