...
function msg_fragment(it) {
    var pager = it.pager;
    return "\n<div v-show=\"" + pager + ".msg &amp;&amp; " + pager + ".state &amp; " + 7 /* MASK_STATUS */ + "\">\n  <div class=\"ui message\"\n      :class=\"{\n        success: " + pager + ".state &amp; " + 1 /* SUCCESS */ + ",\n        error: " + pager + ".state &amp; " + 2 /* ERROR */ + ",\n        warning: " + pager + ".state &amp; " + 4 /* WARNING */ + "\n      }\">\n    <i class=\"close icon\" @click.prevent=\"" + pager + ".msg = null\"></i>\n    <span v-text=\"" + pager + ".msg\"></span>\n  </div>\n</div>";
}
exports.msg_fragment = msg_fragment;
function sort_btn(it) {
    return "\n<button type=\"button\" class=\"ui button\"\n    v-disable=\"2 > " + it.pager + ".size || " + it.pager + ".state &amp; " + 8 /* LOADING */ + "\"\n    v-xon=\"click:" + it.pager + ".$handle(1, \n        (" + it.pager + ".state ^= " + 16 /* DESC */ + common_1.include_if(it.track_clicks, track_clicks, it) + "))\">\n  <i class=\"icon\"\n      v-class=\"" + (it.reverse_icon ? 'down' : 'up') + ":(" + it.pager + ".state &amp; " + 16 /* DESC */ + ") === 0, \n      " + (it.reverse_icon ? 'up' : 'down') + ":" + it.pager + ".state &amp; " + 16 /* DESC */ + "\"></i>\n</button>";
}
