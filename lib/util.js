...
exports.UTC_OFFSET = getUTCOffset(), exports.HOST_RAW_OFFSET = exports.UTC_OFFSET * 60 * 60 * 1000, exports.HOST_RAW_OFFSET_SECONDS = exports.UTC_OFFSET * 60 * 60;
function localToUtc(ts) {
    return ts + exports.HOST_RAW_OFFSET;
}
exports.localToUtc = localToUtc;
function localToUtcSeconds(s) {
    return s + exports.HOST_RAW_OFFSET_SECONDS;
}
exports.localToUtcSeconds = localToUtcSeconds;
function utcToLocal(ts) {
    return ts - exports.HOST_RAW_OFFSET;
}
exports.utcToLocal = utcToLocal;
function utcToLocalSeconds(s) {
    return s - exports.HOST_RAW_OFFSET_SECONDS;
}
exports.utcToLocalSeconds = utcToLocalSeconds;
function newTimeFormatArray(one, ago, fromNow, aSecondAgo, justNow, seconds, minute, minutes, hour, hours, yesterday, tomorrow, days, lastWeek, nextWeek, weeks, lastMonth, nextMonth, months, lastYear, nextYear, years, lastCentury, nextCentury, centuries) {
    return [
...
exports.newTimeFormatArray = newTimeFormatArray;
exports.defaultTimeFormatArray = newTimeFormatArray("1", "ago", "from now", "a second ago", "just now", "seconds", "minute", "minutes", "hour", "hours", "yesterday", "tomorrow", "days", "last week", "next week", "weeks", "last month", "next month", "months", "last year", "next year", "years", "last century", "next century", "centuries");
exports.regexInt = /^-?[0-9]+$/, exports.regexDouble = /^-?[0-9]+(\.[0-9]+)?$/, exports.regexTime = /^[0-2]?[0-9]\:[0-5][0-9](\:[0-5][0-9])?$/, exports.regexDate = /^[1-9][0-9][0-9][0-9]\/[0-9][0-9]\/[0-9][0-9]$/, exports.regexDateTime = /^[1-9][0-9][0-9][0-9]\/[0-9][0-9]\/[0-9][0-9] [0-2]?[0-9]\:[0-5][0-9]\:[0-5][0-9]$/;
exports.screen = {
...
function date_columns() {
    var flags = exports.screen.flags;
    if (flags === 0)
        return 2;
    else if (flags & 4)
        return 14;
...
exports.date_columns = date_columns;
function date_compact_columns() {
    var flags = exports.screen.flags;
    if (flags === 0)
        return 3;
    else if (flags & 4)
...
exports.date_compact_columns = date_compact_columns;
function table_compact_columns() {
    var flags = exports.screen.flags;
    if (flags === 0)
        return 4;
    else if (flags & 4)
...
exports.table_compact_columns = table_compact_columns;
function isInput(el) {
    var tag = el.tagName;
    return tag === 'INPUT' || tag === 'SELECT' || tag === 'TEXTAREA';
}
exports.isInput = isInput;
function findupClass(el, cls, limit) {
    if (hasClassList) {
        do {
...
exports.findupClass = findupClass;
...
exports.getLastChildElement = getLastChildElement;
...
function resolveRelativeElement(el, str) {
...
exports.resolveRelativeElement = resolveRelativeElement;
// TODO optimize: parseInt is not necessary if the length is 1 (simply deduct 48)
function chainResolveRelativeElement(el, array, i) {
    for (var l = array.length; i < l; i++)
        el = resolveRelativeElement(el, array[i]);
    return el;
}
exports.chainResolveRelativeElement = chainResolveRelativeElement;
function resolveElement(el, value, vm) {
...
function resolveElementArray(el, value, selectFromParent, vm) {
...
exports.resolveElementArray = resolveElementArray;
...
function tableUp(pager, col_size, flags, idx, e, toAdd) {
    e.preventDefault();
    var index_hidden;
    if (idx === -1) {
        ...
        if (index_hidden)
            pager.$handle(8 + toAdd, index_hidden - 1);
    }
...
function tableJumpUp(pager, col_size, flags, idx, e, toAdd) {
    e.preventDefault();
    if (idx === -1) {
...
exports.tableJumpUp = tableJumpUp;
function tableDown(pager, col_size, flags, idx, e, toAdd) {
    e.preventDefault();
    if (idx === -1) {
        ...
        if (pager.page !== pager.page_count)
            pager.$handle(9, idx % col_size);
        else
            pager.$handle(9, Math.min(idx % col_size, (pager.size % pager.array.length) - 1));
    }
}
exports.tableDown = tableDown;
function tableJumpDown(pager, col_size, flags, idx, e, toAdd) {
    e.preventDefault();
...
exports.tableJumpDown = tableJumpDown;
function tableLeft(pager, col_size, flags, idx, e, toAdd) {
    e.preventDefault();
    if (idx === -1) {
        pager.$handle(8 + toAdd, Math.min(col_size - 1, pager.index_hidden - 1));
    }
...
function tableJumpLeft(pager, col_size, flags, idx, e, toAdd) {
    e.preventDefault();
    if (idx === -1) {
        pager.$handle(8 + toAdd, 0);
    }
...
exports.tableJumpLeft = tableJumpLeft;
function tableRight(pager, col_size, flags, idx, e, toAdd) {
    e.preventDefault();
    if (idx === -1) {
...
exports.tableRight = tableRight;
function tableJumpRight(pager, col_size, flags, idx, e, toAdd) {
    e.preventDefault();
    if (idx === -1) {
        pager.$handle(8 + toAdd, Math.min(col_size - 1, pager.index_hidden - 1));
    }
...
exports.tableJumpRight = tableJumpRight;
function extractFlagsLen(str) {
...
exports.extractFlagsLen = extractFlagsLen;
function newChangeHandler(self) {
    return function (e) {
...
exports.newChangeHandler = newChangeHandler;
function prevent(e, flags) {
    switch (flags & 3) {
        case 1:
            e.preventDefault();
            return false;
 ...
 * Returns the param
 */
function putArgsTo(param, array, i, flags) {
...
exports.putArgsTo = putArgsTo;
function getFirstVm(el) {
    var vm = el.vue_vm;
    if (!vm) {
...
function vmGetValue(obj, key) {
    var val = getValue(obj, key);
    return val === undefined && obj.$parent
        ? obj.$parent.$get(key)
        : val;
}
exports.vmGetValue = vmGetValue;
function vmLookup(target, p) {
...
exports.vmLookup = vmLookup;
function vmGetHandler(vm) {
    while (!vm.handle)
        vm = vm.$parent;
    return vm.handle;
}
exports.vmGetHandler = vmGetHandler;
...
exports.getObjAndKey = getObjAndKey;
...
function prettyDate(ts, targetTimeFormatArray, ago, fromNow) {
    if (ago === void 0) { ago = 'ago'; }
    if (fromNow === void 0) { fromNow = 'from now'; }
    var seconds = (new Date().getTime() - ts) / 1000, token = ago, list_choice = 1, timeFormatArray = targetTimeFormatArray || exports.defaultTimeFormatArray;
    if (seconds < 0) {
...
exports.toYMD = toYMD;
function toYMDTime(dt, separator) {
...
function isValidDateStr(text) {
    if (!exports.regexDate.test(text) || isNaN(Date.parse(text))) {
        return null;
    }
...
function isValidDateTimeStr(dt) {
...
exports.isValidDateTimeStr = isValidDateTimeStr;
...
function positionTo(relativeTarget, popup) {
...
exports.positionTo = positionTo;
function popTo(relativeTarget, popup) {
...
function newTrie(stem, sorting) {
    return new trie_1.Trie(stem, sorting);
}
exports.newTrie = newTrie;
/*var modalId
