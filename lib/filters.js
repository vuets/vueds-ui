...
export function r2(value, format) {
...
export function hm(value) {
    var hours = Math.floor(value / 60 / 60), minutes = Math.floor((value - (hours * 60 * 60)) / 60);
    return ((hours < 10) ? '0' + hours : hours) + ':' +
        ((minutes < 10) ? '0' + minutes : minutes);
}
export function hm12(value, diff) {
    if (diff)
        value += (60 * parseInt(diff, 10));
    var hours = Math.floor(value / 60 / 60), minutes = Math.floor((value - (hours * 60 * 60)) / 60), suffix;
    if (hours === 0) {
        hours = 12;
        suffix = 'am';
    }
